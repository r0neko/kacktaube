...
namespace Pisstaube.Controllers
{
    ...
    public class BeatmapController : ControllerBase
    {
        ...
        public async Task<ActionResult<string>> GetBeatmapSetByFile (string bmFileName)
        {
            ...
            var bms = (
                from b in _contextFactory.Get ( ).Beatmaps join p in _contextFactory.Get ( ).BeatmapSet on b.ParentSetId equals p.SetId where names.Any (s => s == Regex.Replace ($"{p.Artist} - {p.Title} ({p.Creator}) [{b.DiffName}].osu", @"[^\u0000-\u007F]+", string.Empty)) ||
                names.Any (s => s == Regex.Replace ($"{p.Artist} - {p.Title} ({p.Creator}).osu", @"[^\u0000-\u007F]+", string.Empty)) select p
